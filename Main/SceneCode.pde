...
abstract class Scene {
  ...
  public void assignDir(String direction, boolean canMove){
    ...
    for(int i = this.chain.size()-1; i >= 1; i--){
      ...
      System.out.println(dir + " " + this.chain.get(i).getClass().getSimpleName());
  ...
  public String perDir(String origDir, int[] lastCell){
    ...
    if(directions.size()>1){
      ...
      if((this.grid[lastCell[0]+numExitDir[0]][lastCell[1]+numExitDir[1]] == null ||
         ...
         origDir != this.reverseDir(exitDir)){
        return exitDir;
      }
      else
      ...
      if(exitDir.equals("left") || exitDir.equals("right")){
        if(this.grid[lastCell[0]][lastCell[1]+1] == null || this.grid[lastCell[0]][lastCell[1]+1].getClass().getSimpleName().equals("Block") || this.grid[lastCell[0]][lastCell[1]+1].getClass().getSimpleName().equals("Person"))
          return "down";
        else
          return "up";
      }
      else {
        if(this.grid[lastCell[0]+1][lastCell[1]] == null || this.grid[lastCell[0]+1][lastCell[1]].getClass().getSimpleName().equals("Block") || this.grid[lastCell[0]+1][lastCell[1]].getClass().getSimpleName().equals("Person"))
          return "right";
        else
          return "left";
      }
